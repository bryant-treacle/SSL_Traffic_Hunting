...
# Iteratre throug the ja3_lookup_table
    if ( c$ssl$ja3 in ja3_lookup_table ){
# Stupid work around casused by a type clash assignement.  Needed to convert the output of the table to a string.
	local ja3_lookup_temp = fmt("%s", ja3_lookup_table[c$ssl$ja3]);
        local ja3_lookup_temp_1 = sub(ja3_lookup_temp, /name=/, "");
	local ja3_lookup_temp_2 = sub(ja3_lookup_temp_1, /\[/, "");
	local ja3_lookup_temp_3 = sub(ja3_lookup_temp_2, /\]/, "");
	c$ssl$ja3_desc = ja3_lookup_temp_3;
	}
    else
