...
#############################################################
    if COUNTER < 4800:
        ...
        CN_NAME_SEARCH = elasticsearch.helpers.scan(es,
            index="*:logstash-*",
            preserve_order=True,
            query={"query": {"bool": {"must": [{"term" : {"event_type": "bro_ssl"}}], "filter": [{ "range": {"@timestamp": {"gte": "now-40m", "lte": "now"}}}]}}},
    )
...
##############################################################################################################################################
                    DNS_PARENT_DOMAIN_SEARCH = es.search(index="*:logstash-*", body={"query": {"bool": {"must": [{"wildcard" : {"query.keyword": CN_NAME_WILDCARD}}], "filter": [{ "range": {"@timestamp": {"gte": "now-60m", "lte": "now"}}}]}}}, filter_path=['hits.hits._source.uid'], size=1)
...
##############################################################
                        SSL_SEARCH = es.search(index="*:logstash-*", body={"query": {"bool": {"must": [{"wildcard" : {"certificate_common_name": CN_NAME_WILDCARD}}], "filter": [{ "range": {"@timestamp": {"gte": "now-60m", "lte": "now"}}}]}}}, size=1)
